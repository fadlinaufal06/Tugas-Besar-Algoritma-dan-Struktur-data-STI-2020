...
void TambahOrderan(Queue *Q, ArrNama N, ArrKomponen K, int Jumlah){
    ...
    while (count != Jumlah){
        ...
        while (!IsStackFull(Daftar_Komponen)){
            ...
            // printf("Penambahan komponen ke dalam stack ke %d:\n", count);
            // printf("%d\n", Daftar_Komponen.TOP);
            // printf("%s\n",Daftar_Komponen.T[Top(Daftar_Komponen)].Nama_Komponen);
        }
...
void PrintOrder(Queue Q){ // bisa dipake untuk CHECKORDER
