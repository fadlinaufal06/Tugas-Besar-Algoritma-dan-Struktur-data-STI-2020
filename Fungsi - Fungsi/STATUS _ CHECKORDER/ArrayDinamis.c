 ...
 */
 {
    InsertAt(list, komponen, Length(*list));
 }

void InsertFirst(List *list, Komponen komponen)
{
	InsertAt(list, komponen, 0);
}
...
void DeleteAt(List *list, char* el, IdxType i){
	...
	(*list).A[Idx-1].komponen.Nama_Komponen = "DELETED";
	(*list).A[Idx-1].komponen.Kategori = "DELETED";
	(*list).A[Idx-1].komponen.Harga = 0;
